...
## 3. [Между DC-RTR-2,DC-RTR-1,MSK-RTR,YEKT-RTR настройте удалённый защищённый туннель:] (https://vk.com)
...
## 4. Для обеспечения сетевого взаимодействия настройте протокол динамической маршрутизации OSPF на роутерах:
...
## 5. Для выхода в сеть «Интернет» используйте PAT, настроенный на пограничных роутерах соответственно.

## 6. На базе сервера CLOUD-VM1 необходимо реализовать OpenConnect со следующей логикой работы:
...
## 7. Клиентский компьютер REMOTE-TERMINAL подключается на CLOUD-VM1 через OpenConnect-client. В зависимости от логина, меняется сетевая доступность внутренних ресурсов компании, таблица доступности объясняет это подробнее.
...
## 10. Реализуйте LVM-массив на DC-STORAGE по следующим требованиям:
...
## 11. На машине DC-MAILSERVER уже развернут почтовый сервер организации.
...
## 12. Реализуйте отказоустойчивый шлюз по умолчанию в инфраструктуре ЦОД:
...
## 1. Убедитесь, что домен FreeIPA на MSK-DC1 развернут корректно:
...
## 2. Импортируйте пользователей в домен. Используйте для этого файл по пути - /root/users.csv;
...
## 3. Добавьте компьютеры MSK-ADMINPC и MSK-WORKER в домен FreeIPA.
...
## 7. На машине MSK-GITLAB:

7.1 На машине GITLAB необходимо настроить SAST.

## 8. Настройте DHCP-сервер на машине MSK-RTR:
...
## 1. На сервере YEKT-BILLING развернута самописная система контроля за добычей горных пород, система работает с базой данных на сервере YEKT-DB, необходимо исправить следующие особенности платформы:
...
## 3. Сбор логов организуйте на машине YEKT-WORKER, на рабочем столе локального пользователя - yekt_admin с паролем P@ssw0rd. Директория $HOME/Desktop/Logs/, где NAME имя сервера, с которого был взят лог файл:
...
## 4. На сервере YEKT-RTR разверните подчиненный Центр сертификации:
...
## 5. Испольуя подчиненный Центр сертификации обеспечьте корректную работу YEKT-BILLING приложения по HTTPS.

## 6. Реализуйте веб-сайт, на базе YEKT-RTR:
